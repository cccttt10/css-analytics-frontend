...
interface FontFamiliesProps {
    fontFamilies: CSS.FontFamilyProperty[];
}

const FontFamilies: React.FC<FontFamiliesProps> = ({ fontFamilies = [] }) => (
    <Div sx={{ mt: 5 }}>
        <SectionTitle
        ...
        {uniq(fontFamilies).map(fontFamily => (
            ...
            >
                <Text sx={{ mb: 3, fontWeight: 'bold' }} children={fontFamily} />
                <Div
                    sx={{ fontSize: [4, 5, 6], fontWeight: 'bold' }}
                    style={{ fontFamily }}
                >
